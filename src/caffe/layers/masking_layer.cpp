...
namespace caffe {
	...
	void MaskingLayer<Dtype>::Forward_cpu(
		...
		LOG(INFO) << "MaskingLayer::Forward_cpu() called, this: " << std::to_string((long) this);
	...
	void MaskingLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
		...
		LOG(INFO) << "MaskingLayer::Backward_cpu() called, this=" << std::to_string((unsigned int) this) << ", blobs_[0].mutable_cpu_diff()=" << std::to_string((unsigned int) this->blobs_[0]->mutable_cpu_diff());
