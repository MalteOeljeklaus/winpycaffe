...
namespace caffe {
	...
	void MaskingLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
		...
		stable_prod_grad_ = false; // TODO: read this from proto = this->layer_param_.masking_param().stable_prod_grad();
	}
	...
	void MaskingLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
		...
		if (propagate_down[0]) {
			if (stable_prod_grad_) {
				...
				caffe_mul(top[0]->count(), this->blobs_[0]->cpu_data(), top[0]->cpu_diff(), bottom[0]->mutable_cpu_diff()); // d_i=w.*d_i+1
			}
		}

		// Gradient with respect to weights
		caffe_mul(top[0]->count(), top[0]->cpu_diff(), bottom[0]->cpu_data(), this->blobs_[0]->mutable_cpu_diff()); // = activation*d_i+1
